...
%   Author: Kamil Wojcicki, UTD, July 2011
    ...
    % input validation
    if( isempty(vec) || isempty(Nw) || isempty(Ns) ), error( usage ); endif;
    if( min(size(vec))~=1 ), error( usage ); endif;
    if( Nw==0 || Ns==0 ), error( usage ); endif;
    ...
    if( isempty(window) || ( islogical(window) && ~window ) ), return; endif;
